local config = {
    actionId = 41387, 
    bossName = "medusa",
    bossPosition = Position(31942, 31998, 6), 
    bossArea = {
        fromPos = Position(31924, 31983, 6), 
        toPos = Position(31971, 32020, 6), 
        entrancePos = Position(31938, 31990, 6), 
        exitPosition = Position(32215, 32268, 6) 
    },
    participantsAllowAnyCount = true, 
    participantsPos = {
        Position(31941, 32012, 7),
        Position(31942, 32012, 7),
        Position(31943, 32012, 7), 
        Position(31944, 32012, 7)
    },
    attempts = {
        level = 100, 
        storage = 720251, 
...
local earthlv2Action = Action()

function earthlv2Action.onUse(player, item, fromPos, target, toPos, isHotkey)
    local participants = {}
    for index, pos in pairs(config.participantsPos) do
        ...
        if participant and participant:isPlayer() then
            ...
            player:sendTextMessage(MESSAGE_INFO_DESCR, "Entered on Earth Arena level 2, you have 30 minuts to complet the second level.")
        end
    end
    ...
    boss:registerEvent("earthSecondSystemDeath")
...
earthlv2Action:aid(config.actionId)
earthlv2Action:register()

local creatureEvent = CreatureEvent("earthSecondSystemDeath")
