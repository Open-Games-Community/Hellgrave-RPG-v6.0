local config = {
    actionId = 27386, 
    bossName = "serpent spawn",
    bossPosition = Position(31945, 32001, 7), 
    bossArea = {
        fromPos = Position(31924, 31983, 7), 
        toPos = Position(31966, 32025, 7), 
        entrancePos = Position(31935, 31992, 7), 
        exitPosition = Position(32215, 32268, 6) 
    },
    participantsAllowAnyCount = true, 
    participantsPos = {
        Position(32679, 33093, 6),
        Position(32680, 33093, 6),
        Position(32681, 33093, 6), 
        Position(32682, 33093, 6)
    },
    attempts = {
        level = 100, 
        storage = 720250, 
...
local earthlv1Action = Action()

function earthlv1Action.onUse(player, item, fromPos, target, toPos, isHotkey)
    local participants = {}
    for index, pos in pairs(config.participantsPos) do
        ...
        if participant and participant:isPlayer() then
            ...
            player:sendTextMessage(MESSAGE_INFO_DESCR, "Entered on Earth Arena level 1, you have 30 minuts to complet the first level.")
        end
    end
    ...
    boss:registerEvent("earthOneSystemDeathArkeron")
...
earthlv1Action:aid(config.actionId)
earthlv1Action:register()

local creatureEvent = CreatureEvent("earthOneSystemDeathArkeron")
