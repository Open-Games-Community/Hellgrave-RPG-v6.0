local config = {
    actionId = 41388, 
    bossName = "ripper spectre",
    bossPosition = Position(31947, 31999, 5), 
    bossArea = {
        fromPos = Position(31932, 31983, 5), 
        toPos = Position(31971, 32017, 5), 
        entrancePos = Position(31951, 31987, 5), 
        exitPosition = Position(32215, 32268, 6) 
    },
    participantsAllowAnyCount = true, 
    participantsPos = {
        Position(31929, 32012, 6),
        Position(31930, 32012, 6),
        Position(31931, 32012, 6), 
        Position(31932, 32012, 6)
    },
    attempts = {
        level = 100, 
        storage = 720252, 
...
local earthlv3Action = Action()

function earthlv3Action.onUse(player, item, fromPos, target, toPos, isHotkey)
    local participants = {}
    for index, pos in pairs(config.participantsPos) do
        ...
        if participant and participant:isPlayer() then
            ...
            player:sendTextMessage(MESSAGE_INFO_DESCR, "Entered on Earth Arena level 3, you have 30 minuts to complet the thirst level.")
        end
    end
    ...
    boss:registerEvent("earthThirstSystemDeath")
...
earthlv3Action:aid(config.actionId)
earthlv3Action:register()

local creatureEvent = CreatureEvent("earthThirstSystemDeath")
