local config = {
    ...
    bossPosition = Position(31937, 31941, 7), 
    bossArea = {
        fromPos = Position(31920, 31931, 7), 
        toPos = Position(31971, 31967, 7), 
        entrancePos = Position(31940, 31935, 7), 
        exitPosition = Position(32215, 32268, 6) 
    },
    participantsAllowAnyCount = true, 
    participantsPos = {
        Position(32211, 32272, 6),
        Position(32212, 32272, 6),
        Position(32213, 32272, 6), 
        Position(32214, 32272, 6)
    },
    attempts = {
        level = 100, 
        storage = 720253, 
...
local firelvl1Action = Action()

function firelvl1Action.onUse(player, item, fromPos, target, toPos, isHotkey)
    local participants = {}
    for index, pos in pairs(config.participantsPos) do
        ...
        if participant and participant:isPlayer() then
            ...
            player:sendTextMessage(MESSAGE_INFO_DESCR, "Entered on Fire Arena level 1, you have 30 minuts to complet the first level.")
        end
    end
    ...
    boss:registerEvent("fireFirstSystemDeath")
...
firelvl1Action:aid(config.actionId)
firelvl1Action:register()

local creatureEvent = CreatureEvent("fireFirstSystemDeath")
