...
function firstWorldBoss.onStepIn(player, item, fromPosition)
    if player:getStorageValue(Storage.WorldBosses.Unlock.Ossirian) < 1 then
        player:setStorageValue(Storage.WorldBosses.Unlock.Ossirian, 1)
        player:sendTextMessage(MESSAGE_INFO_DESCR, "You have found some rare Traces, check your questlog for additional information.")
        return true
    elseif player:getStorageValue(Storage.WorldBosses.Unlock.Ossirian) == 1 or player:getStorageValue(Storage.WorldBosses.Unlock.Ossirian) == 2 then
        return false
    else
