...
function firstWorldBoss.onStepIn(player, item, fromPosition)
    if player:getStorageValue(Storage.WorldBosses.Unlock.Archaedas) < 1 then
        ...
        return true
    elseif player:getStorageValue(Storage.WorldBosses.Unlock.Archaedas) == 1 or player:getStorageValue(Storage.WorldBosses.Unlock.Archaedas) == 2 then
        return false
    else
