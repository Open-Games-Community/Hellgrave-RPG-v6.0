...
function entrance.onStepIn(creature, item, position, fromPosition)
	...
	if item:getActionId() == 27525 then
		if rankIsEqualOrHigher(getRankTask(creature), configStorages.rank) and taskPoints_get(creature) >= 5 and configStorages.parangonStor >= 40 and configStorages.gearScoreStor >= 2800 then
			player:teleportTo(Position(32756, 32517, 7))
			...
			player:teleportTo(Position(32218, 32465, 12))
			player:sendTextMessage(MESSAGE_INFO_DESCR, "You find the Dreadlord Portal, you need at least Ancestral Rank 'Gold', Ancestral Points: 5, Parangon Level: 40 and Gear Score: 2800.")
			return true
		end
	elseif item:getActionId() == 27526 then
		if player then
			player:teleportTo(Position(32218, 32465, 12))
			player:getPosition():sendMagicEffect(CONST_ME_TELEPORT)
			return true
