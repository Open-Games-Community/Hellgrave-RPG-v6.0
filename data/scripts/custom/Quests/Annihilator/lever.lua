...
local quest_range = {fromPos = Position(32334, 32348, 13), toPos = Position(32342, 32352, 13)} -- see image in thread for explanation

local exit_position = Position(32348, 32363, 13) -- Position completely outside the quest area

local theAnnihilator = Action()
...
function theAnnihilator.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    if player:getStorageValue(config.cooldown_storage) >= os.time() then
        player:sendTextMessage(MESSAGE_INFO_DESCR, "Try Again in 1 Hour.")
        return true
    end
    ...
    for i = 1, #monsters do
        ...
        if not Tile(toPos) then
            print(">> ERROR: Anihilator tile does not exist for Position(" .. toPos.x .. ", " .. toPos.y .. ", " .. toPos.z .. ").")
            return player:sendCancelMessage("There is an issue with this quest. Please contact an administrator.")
        end
        removeBoss()
    ...
    for i = 1, #participants do
        ...
        player:sendTextMessage(MESSAGE_INFO_DESCR, "You have 10 minutes to kill all Demons. Otherwise you will lose that chance and will be kicked out.")
    end
...
theAnnihilator:uid(30025)
theAnnihilator:register()
