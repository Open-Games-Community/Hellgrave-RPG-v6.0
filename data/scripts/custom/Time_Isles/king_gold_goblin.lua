local config = {
    actionId = 37312, 
    bossName = "king gold goblin",
    bossPosition = Position(32071, 31894, 2), 
    bossArea = {
        fromPos = Position(32067, 31890, 2), 
        toPos = Position(32078, 31897, 2), 
        entrancePos = Position(32071, 31913, 7), 
        exitPosition = Position(32258, 32557, 6) 
    },
    participantsAllowAnyCount = true, 
    participantsPos = {
        Position(32071, 31917, 7)
    },
    attempts = {
        level = 100, 
        storage = 580203, 
    ...
    createTeleportPos = Position(32068, 31894, 2), 
...
function riftNumberFourAction.onUse(player, item, fromPos, target, toPos, isHotkey)
    ...
    boss:registerEvent("kingGoldGoblin")
    ...
    config.kickEventId = addEvent(function ()
        for _, spectator in pairs(getSpectators()) do
            if spectator:isPlayer() then
                ...
                spectator:sendTextMessage(MESSAGE_INFO_DESCR, "It's been a long time and you haven't managed to defeat King Gold Goblin Boss.")
            else
                spectator:remove()
            end
        end
...
local creatureEvent = CreatureEvent("kingGoldGoblin")
