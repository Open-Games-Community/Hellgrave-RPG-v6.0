local config = {
    actionId = 37313, 
    bossName = "king gather goblin",
    bossPosition = Position(32200, 31979, 7), 
    bossArea = {
        fromPos = Position(32117, 31892, 7), 
        toPos = Position(32224, 31992, 7), 
        entrancePos = Position(32167, 31941, 7), 
        exitPosition = Position(32258, 32557, 6) 
    },
    participantsAllowAnyCount = true, 
    participantsPos = {
        Position(32181, 31882, 7)
    },
    attempts = {
        level = 100, 
        storage = 580205, 
    ...
    createTeleportPos = Position(32205, 31979, 7), 
    teleportToPosition = Position(32258, 32557, 6), 
...
local riftNumberFourAction = Action()

function riftNumberFourAction.onUse(player, item, fromPos, target, toPos, isHotkey)
    local participants = {}
    ...
    boss:registerEvent("kingGatherGoblinDeath")
    ...
    config.kickEventId = addEvent(function ()
        for _, spectator in pairs(getSpectators()) do
            if spectator:isPlayer() then
                ...
                spectator:sendTextMessage(MESSAGE_INFO_DESCR, "It's been a long time and you haven't managed to defeat King Gather Goblin Boss.")
            else
                spectator:remove()
            end
        end
...
riftNumberFourAction:aid(config.actionId)
riftNumberFourAction:register()

local creatureEvent = CreatureEvent("kingGatherGoblinDeath")
