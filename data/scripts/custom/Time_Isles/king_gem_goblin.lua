local config = {
    actionId = 37311, 
    bossName = "king gem goblin",
    bossPosition = Position(31981, 31873, 10), 
    bossArea = {
        fromPos = Position(31964, 31860, 10), 
        toPos = Position(32000, 31891, 10), 
        entrancePos = Position(31976, 31901, 9), 
        exitPosition = Position(32258, 32557, 6) 
    },
    participantsAllowAnyCount = true, 
    participantsPos = {
        Position(31992, 31901, 10)
    },
    attempts = {
        level = 100, 
        storage = 580201, 
    ...
    createTeleportPos = Position(31983, 31872, 10), 
...
function riftNumberFourAction.onUse(player, item, fromPos, target, toPos, isHotkey)
    ...
    boss:registerEvent("kingGemGoblinDeath")
    ...
    config.kickEventId = addEvent(function ()
        for _, spectator in pairs(getSpectators()) do
            if spectator:isPlayer() then
                ...
                spectator:sendTextMessage(MESSAGE_INFO_DESCR, "It's been a long time and you haven't managed to defeat King Gem Goblin Boss.")
            else
                spectator:remove()
            end
        end
...
local creatureEvent = CreatureEvent("kingGemGoblinDeath")
