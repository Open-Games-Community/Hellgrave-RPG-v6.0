local config = {
    actionId = 43016, 
    bossName = "oculus worm [**]",
    bossPosition = Position(29622, 32215, 9), 
    bossArea = {
        fromPos = Position(29604, 32205, 9), 
        toPos = Position(29636, 32232, 9), 
        entrancePos = Position(29616, 32226, 9), 
    ...
    createTeleportPos = Position(29612, 32218, 9), 
...
local elderRiftFirst = Action()

function elderRiftFirst.onUse(player, item, fromPos, target, toPos, isHotkey)
    local participants = {}
    ...
    boss:registerEvent("elderRiftSystemDeath")
...
elderRiftFirst:aid(config.actionId)
elderRiftFirst:register()

local creatureEvent = CreatureEvent("elderRiftSystemDeath")
