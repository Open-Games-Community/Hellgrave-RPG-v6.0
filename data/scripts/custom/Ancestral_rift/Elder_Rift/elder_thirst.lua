local config = {
    actionId = 43018, 
    bossName = "raven the distorted [**]",
    bossPosition = Position(29625, 32173, 9),
    bossArea = {
        fromPos = Position(29603, 32160, 9),
        toPos = Position(29649, 32192, 9), 
        entrancePos = Position(29612, 32182, 9), 
        exitPosition = Position(31825, 32408, 8) 
    },
    participantsAllowAnyCount = true, 
    participantsPos = {
        Position(29390, 32248, 4),
        Position(29390, 32249, 4),
        Position(29390, 32250, 4), 
        Position(29390, 32251, 4) 
    },
    attempts = {
        level = 450, 
        storage = 710011, 
    ...
    createTeleportPos = Position(29622, 32168, 9), 
...
local elderRiftThirst = Action()

function elderRiftThirst.onUse(player, item, fromPos, target, toPos, isHotkey)
    local participants = {}
    ...
    boss:registerEvent("elderRiftThirstSystemDeath")
...
elderRiftThirst:aid(config.actionId)
elderRiftThirst:register()

local creatureEvent = CreatureEvent("elderRiftThirstSystemDeath")
