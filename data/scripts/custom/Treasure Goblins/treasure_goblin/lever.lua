local config = {
    ...
    cooldown_storage = 532100,
    duration = 8, -- time till reset, in minutes (lever cooldown)
...
local player_positions = {
    [1] = {fromPos = Position(32015, 32356, 1), toPos = Position(31977, 31996, 8)}
}

local monsters = {
    [1] = {pos = Position(31964, 32009, 11), name = "Treasure Goblin the first"}
}
local quest_range = {fromPos = Position(31951, 31996, 11), toPos = Position(31977, 32018, 11)} -- see image in thread for explanation

local exit_position = Position(32015, 32356, 1) -- Position completely outside the quest area
...
local specs, spec = Game.getSpectators(Position(33919, 31646, 8), false, false, 18, 18, 18, 18)
    for j = 1, #specs do
        spec = specs[j]
        if spec:getName():lower() == 'Treasure Goblin the first' then
            spec:remove()
        end
    end
end
...
function lever.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    for i = 1, #player_positions do
        ...
        if not tile then
            print(">> ERROR: Treasure Goblin the first tile does not exist for Position(" .. fromPos.x .. ", " .. fromPos.y .. ", " .. fromPos.z .. ").")
    ...
    for i = 1, #monsters do
        ...
        if not Tile(toPos) then
            print(">> ERROR: Treasure Goblin the first tile does not exist for Position(" .. toPos.x .. ", " .. toPos.y .. ", " .. toPos.z .. ").")
            return player:sendCancelMessage("There is an issue with this quest. Please contact an administrator.")
        end
        removeBoss()
    ...
    for i = 1, #participants do
        ...
        player:sendTextMessage(MESSAGE_INFO_DESCR, "You have 8 minutes to kill and loot this boss. Otherwise you will lose that chance and will be kicked out.")
    end
