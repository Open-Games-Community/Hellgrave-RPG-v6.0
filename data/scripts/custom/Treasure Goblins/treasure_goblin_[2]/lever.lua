...
local player_positions = {
    [1] = {fromPos = Position(32015, 32358, 1), toPos = Position(32046, 31997, 8)}
}

local monsters = {
    [1] = {pos = Position(32033, 32010, 11), name = "Treasure Goblin the second"}
}
local quest_range = {fromPos = Position(32021, 31998, 11), toPos = Position(32045, 32019, 11)} -- see image in thread for explanation

local exit_position = Position(32015, 32358, 1) -- Position completely outside the quest area
...
local specs, spec = Game.getSpectators(Position(33919, 31646, 8), false, false, 18, 18, 18, 18)
    for j = 1, #specs do
        spec = specs[j]
        if spec:getName():lower() == 'Treasure Goblin the second' then
            spec:remove()
        end
    end
end
...
function lever.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    for i = 1, #player_positions do
        ...
        if not tile then
            print(">> ERROR: Treasure Goblin the second tile does not exist for Position(" .. fromPos.x .. ", " .. fromPos.y .. ", " .. fromPos.z .. ").")
    ...
    for i = 1, #monsters do
        ...
        if not Tile(toPos) then
            print(">> ERROR: Treasure Goblin the second tile does not exist for Position(" .. toPos.x .. ", " .. toPos.y .. ", " .. toPos.z .. ").")
            return player:sendCancelMessage("There is an issue with this quest. Please contact an administrator.")
        end
        removeBoss()
    ...
    for i = 1, #participants do
        ...
        player:sendTextMessage(MESSAGE_INFO_DESCR, "You have 10 minutes to kill and loot this boss. Otherwise you will lose that chance and will be kicked out.")
    end
