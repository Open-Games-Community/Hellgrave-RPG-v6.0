local config = {
    ...
    cooldown_storage = 532103,
    duration = 16, -- time till reset, in minutes (lever cooldown)
...
local player_positions = {
    [1] = {fromPos = Position(32019, 32358, 1), toPos = Position(32189, 31998, 8)}
}

local monsters = {
    [1] = {pos = Position(32176, 32011, 11), name = "Treasure Goblin the fourth"}
}
local quest_range = {fromPos = Position(32163, 31998, 11), toPos = Position(32191, 32022, 11)} -- see image in thread for explanation

local exit_position = Position(32019, 32358, 1) -- Position completely outside the quest area
...
local specs, spec = Game.getSpectators(Position(33919, 31646, 8), false, false, 18, 18, 18, 18)
    for j = 1, #specs do
        spec = specs[j]
        if spec:getName():lower() == 'Treasure Goblin the fourth' then
            spec:remove()
        end
    end
end
...
function lever.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    for i = 1, #player_positions do
        ...
        if not tile then
            print(">> ERROR: Treasure Goblin the fourth tile does not exist for Position(" .. fromPos.x .. ", " .. fromPos.y .. ", " .. fromPos.z .. ").")
    ...
    for i = 1, #monsters do
        ...
        if not Tile(toPos) then
            print(">> ERROR: Treasure Goblin the fourth tile does not exist for Position(" .. toPos.x .. ", " .. toPos.y .. ", " .. toPos.z .. ").")
            return player:sendCancelMessage("There is an issue with this quest. Please contact an administrator.")
        end
        removeBoss()
    ...
    for i = 1, #participants do
        ...
        player:sendTextMessage(MESSAGE_INFO_DESCR, "You have 16 minutes to kill and loot this boss. Otherwise you will lose that chance and will be kicked out.")
    end
