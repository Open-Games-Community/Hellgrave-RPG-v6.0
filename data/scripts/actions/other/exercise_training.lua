...
function exerciseTraining.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if target:isItem() and (isInArray(houseDummies, targetId) or isInArray(freeDummies, targetId)) then
		if onExerciseTraining[playerId] then
			player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "This exercise dummy can only be used after a 5 second cooldown.")
		...
		if not onExerciseTraining[playerId].event then
			...
			player:setStorageValue(Storage.isTraining, os.time() + 5)
		end
		return true
	end
	return false
