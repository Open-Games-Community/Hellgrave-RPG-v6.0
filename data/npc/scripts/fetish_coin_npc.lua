...
local shop = { -- 175 gps 1 fetish coin 
	{id=14342, buy=3, sell=0, name='Wood Branch'},
	{id=8707, buy=1, sell=0, name='Thick Branch'},
	{id=8701, buy=1, sell=0, name='Thin Branches'},
	{id=8710, buy=4, sell=0, name='Big Branch'},
	{id=14346, buy=7, sell=0, name='Tree Trunk'},
	{id=8977, buy=18, sell=0, name='Enormous Tree Trunk'},
...
local function onSell(cid, item, subType, amount, ignoreCap, inBackpacks)
	...
	if not ignoreCap and player:getFreeCapacity() < ItemType(itemsTable[item].id):getWeight(amount) then
		...
		if player:addItem(26319, amount* itemsTable[item].sell, amount* itemsTable[item].sell) then
			return player:sendTextMessage(MESSAGE_TRADE,
						"Sell "..amount.."x "..itemsTable[item].name.." for "..amount* itemsTable[item].sell.." Fetish Coins.")
end
return true
end
...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "Trade") then
		...
		npcHandler:say("You can buy some lumberjack materials for Fetish Coins.", cid)
	end
	return true
end
