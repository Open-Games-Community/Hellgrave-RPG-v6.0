...
bool ConfigManager::load()
{
	...
	if (!loaded) { //info that must be loaded one time (unless we reset the modules involved)
		...
		string[MAP_NAME] = getGlobalString(L, "mapName", "hellgrave");
		string[MAP_AUTHOR] = getGlobalString(L, "mapAuthor", "Hellgrave");
		...
		string[MAP_CUSTOM_AUTHOR] = getGlobalString(L, "mapCustomAuthor", "Hellgrave");
		...
		string[MYSQL_DB] = getGlobalString(L, "mysqlDatabase", "hellgrave");
	...
	boolean[EMOTE_SPELLS] = getGlobalBoolean(L, "emoteSpells", true);
